...
class GroundingDataset(Dataset):
    ...
    def _getitem_coarse2refine_sft(self, idx):
        ...
        gt_start, gt_end = normalize_single_span(anno["span"])
        if not 0 <= gt_start <= gt_end <= duration:
            raise ValueError(
                f"Invalid continuous GT [{gt_start}, {gt_end}] for duration {duration}."
            )
        ...
        text_target_frame_bins = quantize_time_bins(
            frame_timestamps,
            duration,
            time_bin_count=getattr(self.data_args, "time_bin_count", 301),
        )
        frame_labels = build_frame_labels(
            ...
            coverage_threshold=getattr(
                self.data_args, "frame_label_coverage_threshold", 0.5
            ),
            eps=getattr(self.data_args, "time_refine_eps", 1e-6),
        ).labels
        response = build_vtg_target(frame_labels, text_target_frame_bins)
        ...
        if video_grid_thw is None:
            raise ValueError("Processor output is missing video_grid_thw.")
        ...
        if int(video_grid_thw[0, 0].item()) != frame_timestamps.numel():
            raise ValueError(
                "Temporal block metadata does not match video_grid_thw: "
                f"timestamps={frame_timestamps.numel()}, "
                f"grid_t={int(video_grid_thw[0, 0].item())}."
            )

        inputs["frame_bin_ids"] = text_target_frame_bins
        inputs["frame_timestamps"] = frame_timestamps
        inputs["frame_labels"] = frame_labels.to(dtype=torch.long)
        inputs["frame_valid_mask"] = torch.ones(
            frame_timestamps.shape, dtype=torch.bool
        )
        inputs["gt_start"] = torch.tensor([gt_start], dtype=torch.float32)
        inputs["gt_end"] = torch.tensor([gt_end], dtype=torch.float32)
