...
class TrainingArguments(HFTrainingArguments):
    ...
    time_token_lr: Optional[float] = field(
        default=None,
        metadata={"help": "Learning rate for the target model input embeddings."},
    )
...
class GRPOArguments(GRPOConfigTRL):
    ...
    def __post_init__(self):
        if not _TRL_AVAILABLE:
            raise ImportError(
                "GRPOArguments requires the trl package; SFT paths do not depend on trl."
            )
        super().__post_init__()
...
class DataArguments:
    ...
    frame_label_coverage_threshold: float = field(
        default=0.5,
        metadata={
            "help": "Minimum temporal-support coverage for a foreground block."
        },
    )
    time_refine_eps: float = field(default=1e-6)
