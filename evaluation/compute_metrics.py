...
if __name__ == "__main__":
    ...
    num_annos = len(datas)
    if num_annos == 0:
        raise ValueError("Prediction file contains no annotations.")
    ious = []
    status_counts = {"no_foreground": 0, "invalid_generation": 0}
    ...
    RESULT_STR += (
        "\nno_foreground_rate: {0}\ninvalid_generation_rate: {1}"
    ).format(
        status_counts["no_foreground"] * 100 / num_annos,
        status_counts["invalid_generation"] * 100 / num_annos,
    )
    print(RESULT_STR)
